...
| **POST** `/api/login`               | `{ "username": "string", "password": "string" }` | None                     | Login a user. Requires `username` and `password`. Returns both a JWT token and a refresh token upon success in the cookie.                                        |
